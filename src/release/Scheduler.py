...
class FinalScheduler(Scheduler):
    ...
    def schedule(self, logical_clock: int, request_list: list, driver_statues: list) -> list:
        ...
        request_list_ = []
        driver_statues_ = []
        for json_str in request_list:
            request_list_.append(json.loads(s=json_str))
        for json_str in driver_statues:
            driver_statues_.append(json.loads(s=json_str))
        # 任务分类
        self.method.task_assign(logical_clock, request_list_, driver_statues_)
        # 处理任务
        dict_ans = self.method.op_task(logical_clock, request_list_, driver_statues_)
        # dict转json
        for dic in dict_ans:
            self.ans.append(json.dumps(dic))
        return self.ans


class LossFuncMethod3:
    def __init__(self):
    ...
    def find_best_driver(self, request_list):
        ...
        for request_json in request_list:
            ...
            # 按照tasksize排序
            avail_driver_json_list.sort(key=lambda x: (x['TaskSize']+1)/(x['Capacity']+1))  # 分母为0会寄，还没改
    ...
    def gen_value_1(self):
        ...
        for ID in list(self.request.keys()):
            ...
            self.request[ID]['size_value'] = self.request[ID]['RequestSize'] % 50
    ...
    def op_task(self, logical_clock: int, request_list: list, driver_statues: list) -> list:
        ...
        sort_list = sorted(list(self.request.values()), key=functools.cmp_to_key(cmp))
        # 为每条记录生成选择驱动器列表
        self.find_best_driver(sort_list)
...
# 排序比较函数
def cmp(x: dict, y: dict):
    if x['value'] < y['value'] or x['value'] == y['value'] and x['size_value'] < y['size_value']:
        return -1
    else:
        return 1
