...
class LossFuncMethod1(AlgBase):
    ...
    def gen_value_1(self):
        ...
        Returns:
            None
        """
        # 计算此时距离丢弃还剩下多少时间并把所有分数相加方便后面归一化,下同.还没改完！
    ...
    def op_task(self, logical_clock: int, request_list: list, driver_statues: list) -> list:
        """
        对task_assign函数中预存储好的数据进行分配。思路是根据请求的value进行顺序处理
        ...
        # 输出调试
        self.logger.disp_list(sort_list, 'sorted by value:')
        for json in sort_list:
            for avail_driver in json['Driver']:
